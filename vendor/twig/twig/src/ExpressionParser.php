...
class ExpressionParser
{
    ...
    private function isUnary(\Twig\Token $token) : bool
    {
        return $token->test(
            /* Token::OPERATOR_TYPE */
            8
    ...
    private function isBinary(\Twig\Token $token) : bool
    {
        return $token->test(
            /* Token::OPERATOR_TYPE */
            8
        ...
        switch ($token->getType()) {
            ...
            default:
                if ($token->test(
                    ...
                    throw new SyntaxError(\sprintf('Unexpected token "%s" of value "%s".', \Twig\Token::typeToEnglish($token->getType()), $token->getValue()), $token->getLine(), $this->parser->getStream()->getSourceContext());
                }
        }
    ...
    public function parseHashExpression()
    {
        ...
        while (!$stream->test(
            ...
            if ($token = $stream->nextIf(
                ...
                throw new SyntaxError(\sprintf('A hash key must be a quoted string, a number, a name, or an expression enclosed in parentheses (unexpected token "%s" of value "%s".', \Twig\Token::typeToEnglish($current->getType()), $current->getValue()), $current->getLine(), $stream->getSourceContext());
            }
            $stream->expect(
                /* Token::PUNCTUATION_TYPE */
                9,
    ...
    public function getFunctionNode($name, $line)
    {
        switch ($name) {
            ...
            case 'attribute':
                ...
                return new GetAttrExpression($args->getNode(0), $args->getNode(1), \count($args) > 2 ? $args->getNode(2) : null, \Twig\Template::ANY_CALL, $line);
            default:
    ...
    public function parseSubscriptExpression($node)
    {
        ...
        if ('.' == $token->getValue()) {
            $token = $stream->next();
            if (5 == $token->getType() || 6 == $token->getType() || 8 == $token->getType() && \preg_match(\Twig\Lexer::REGEX_NAME, $token->getValue())) {
                ...
                throw new SyntaxError(\sprintf('Expected name or number, got value "%s" of type %s.', $token->getValue(), \Twig\Token::typeToEnglish($token->getType())), $lineno, $stream->getSourceContext());
            }
    ...
    public function parseAssignmentExpression()
    {
        ...
        while (\true) {
            ...
            if ($stream->test(
                /* Token::OPERATOR_TYPE */
                8
            ) && \preg_match(\Twig\Lexer::REGEX_NAME, $token->getValue())) {
                // in this context, string operators are variable names
                $this->parser->getStream()->next();
            } else {
                $stream->expect(
    ...
    private function getTestNodeClass(\Twig\TwigTest $test) : string
    {
...
\class_alias('Twig\\ExpressionParser', 'Torch202401\\Twig_ExpressionParser');
