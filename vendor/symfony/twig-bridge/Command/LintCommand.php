 ...
 */
namespace Torch202308\Symfony\Bridge\Twig\Command;

use Torch202308\Symfony\Component\Console\Attribute\AsCommand;
use Torch202308\Symfony\Component\Console\CI\GithubActionReporter;
...
class LintCommand extends Command
{
    ...
    {
        $this->addOption('format', null, InputOption::VALUE_REQUIRED, \sprintf('The output format ("%s")', \implode('", "', $this->getAvailableFormatOptions())))->addOption('show-deprecations', null, InputOption::VALUE_NONE, 'Show deprecations as errors')->addArgument('filename', InputArgument::IS_ARRAY, 'A file, a directory or "-" for reading from STDIN')->setHelp(<<<'EOF'
...
EOF
    ...
    private function display(InputInterface $input, OutputInterface $output, SymfonyStyle $io, array $files) : int
    {
        switch ($this->format) {
            ...
            case 'github':
                return $this->displayTxt($output, $io, $files, \true);
            default:
    ...
    private function renderException(SymfonyStyle $output, string $template, Error $exception, string $file = null, GithubActionReporter $githubReporter = null) : void
    {
        ...
        ($githubReporter2 = $githubReporter) ? $githubReporter2->error($exception->getRawMessage(), $file, $line <= 0 ? null : $line) : null;
        if ($file) {
